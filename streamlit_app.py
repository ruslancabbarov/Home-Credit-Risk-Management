...
def show_result(result: dict, credit_amount: float):
    ...
    if decision == 'APPROVE':
        ...
        <div class="approve-card">
            ...
            <p>Müştəri aşağı risk qrupundadır. Kredit avtomatik təsdiq edilə bilər.</p>
        </div>""", unsafe_allow_html=True)
    elif decision == 'REVIEW':
        st.markdown(f"""
        <div class="review-card">
            <h2>⚠️ ƏLAVƏ YOXLAMA LAZIMDIR</h2>
            <p>Müştəri orta risk qrupundadır. Kredit mütəxəssisi nəzərdən keçirməlidir.</p>
        </div>""", unsafe_allow_html=True)
    else:
        st.markdown(f"""
        <div class="decline-card">
            <h2>❌ KREDİT İMTİNA EDİLDİ</h2>
            <p>Müştəri yüksək risk qrupundadır. Kredit verilmir.</p>
        </div>""", unsafe_allow_html=True)
    ...
    # Expected Loss formulu
    with st.expander("💰 Gözlənilən İtki Hesablaması"):
        st.latex(r"EL = PD \times LGD \times EAD")
        col1, col2, col3, col4 = st.columns(4)
        col1.metric("PD (Default ehtimalı)", f"{proba:.4f}")
        col2.metric("LGD (İtki nisbəti)",    "45%")
        col3.metric("EAD (Kredit məbləği)",  f"{credit_amount:,.0f} AZN")
        col4.metric("EL (Gözlənilən itki)",  f"{EL:,.0f} AZN")

# ════════════════════════════════════════════════════════════
# TAB 1 — SK_ID ilə axtar
# ════════════════════════════════════════════════════════════
with tab1:
    st.subheader("🔍 Mövcud Müştəri Qiymətləndirmə")
    st.info("Müştərinin SK_ID_CURR nömrəsini daxil edin. "
            "Sistem bütün büro məlumatlarını avtomatik çəkəcək.")
    ...
    with col1:
        sk_id = st.number_input(
            ...
            value=100002,
            step=1,
            help="Nümunə ID-lər: 100002, 100003, 100004"
        )
    with col2:
        st.markdown("<br>", unsafe_allow_html=True)
    ...
    if search_btn:
            ...
            try:
                ...
                if r.status_code == 404:
                    st.error(f"❌ SK_ID={sk_id} tapılmadı! "
                             f"Nümunə: 100002, 100003, 100004")
                    st.stop()
                elif r.status_code != 200:
                    st.error(f"Xəta: {r.json().get('detail', 'Bilinməyən')}")
                    st.stop()
        ...
        with st.expander("👤 Müştəri Profili", expanded=True):
            ...
            with c2:
                st.write(f"**Gəlir:** "
                         f"{result.get('amt_income', 0):,.0f} AZN")
                st.write(f"**Kredit:** "
                         f"{result.get('amt_credit', 0):,.0f} AZN")
            with c3:
...
with tab2:
    ...
    with st.form("new_customer_form"):
        ...
        with col2:
            st.markdown("**💰 Maliyyə**")
            income    = st.number_input("Gəlir (AZN)",
                                         10000, 5000000, 150000, 5000)
            credit    = st.number_input("Kredit (AZN)",
                                         10000, 5000000, 300000, 10000)
            annuity   = st.number_input("Aylıq ödəniş (AZN)",
                                         1000, 200000, 15000, 1000)
            contract  = st.selectbox("Kredit növü",
        ...
        with col3:
            st.markdown("**🏢 İş & Büro**")
            years_emp = st.number_input("İş təcrübəsi (il)",
                                         0.0, 50.0, 5.0)
        ...
        st.markdown("**🔢 Xarici Skorlar (EXT_SOURCE)**")
        c1, c2, c3 = st.columns(3)
        with c1:
        ...
        with st.spinner("Qiymətləndirilir..."):
            try:
                ...
                if r.status_code != 200:
                    st.error(f"Xəta: {r.json().get('detail')}")
                    st.stop()
                result = r.json()
            except Exception as e:
                st.error(f"API xətası: {e}")
